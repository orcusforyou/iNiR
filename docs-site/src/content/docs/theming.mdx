...
iNiR uses its built-in Python Material You pipeline to extract colors from your wallpaper.

### How It Works

1. You set a wallpaper
2. The Python color pipeline analyzes the image
...
Themes are defined by iNiR presets and generated palette contracts. To create a custom theme:

1. Copy an existing theme template
2. Modify colors
3. Save it in your iNiR theme/templates setup
...
See the iNiR theming docs for the current palette contract and template syntax.
...
iNiR applies GTK themes via rendered iNiR templates. Your GTK apps will match your shell theme.
...
1. Check that the iNiR Python venv is installed
2. Verify wallpaper path is correct
3. Restart iNiR
4. Check logs for color-generation errors
...
2. Verify iNiR theming templates are installed
3. Restart GTK apps

## Advanced Theming

For complete control, edit the installed iNiR theming templates directly:

```bash
nano ~/.config/matugen/templates.json
```

Also review `~/.config/matugen/templates.json` and the template files under `~/.config/matugen/templates/`.
